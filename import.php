...
if( isset( $_FILES[ 'import_tasks' ] ) && isset( $_POST[ 'wpcrm_system_import_tasks_nonce' ] ) ){
	if( wp_verify_nonce( $_POST[ 'wpcrm_system_import_tasks_nonce' ], 'wpcrm-system-import-tasks-nonce' ) ){
		...
		if( !in_array( $file_type, $csv_types ) ){
			$errors[] = __( 'File not allowed, please use a CSV file.', 'wp-crm-system-import-tasks' );
		}
		if( $file_size > return_bytes( ini_get( 'upload_max_filesize' ) ) ){
			$errors[] = __( 'File size must be less than', 'wp-crm-system-import-tasks' ) . ini_get( 'upload_max_filesize' );
		}				
		if( empty ( $errors ) == true ){
					...
					if( null == get_page_by_title( $title, OBJECT, 'wpcrm-task' ) ) {
						...
						if( $taskCategories != '' ) {
							...
							if ( is_wp_error( $taskTypes ) ) {
								$error[] = __( 'There was an error with the categories and they could not be set.', 'wp-crm-system-import-tasks' );
							}
						}
						if( $custom_fields ){
			...
			<div id="message" class="updated">
				<p><strong><?php _e( 'Tasks uploaded. ', 'wp-crm-system-import-tasks' ); echo $count_added; _e( ' added. ', 'wp-crm-system-import-tasks' ); echo $count_skipped; _e( ' skipped.', 'wp-crm-system-import-tasks' ); ?> </strong></p>
			</div>
		<?php } else { ?>
...
<div class="wrap">
	<div>
		<h2><?php _e( 'Import Tasks From CSV', 'wp-crm-system-import-tasks' ); ?></h2>
				...
				<form id="wpcrm_import_tasks" name="wpcrm_import_tasks" method="post" action="" enctype="multipart/form-data">
					<tr>
						...
						<td>
							...
							$link = sprintf( wp_kses( __( 'Important: Please make sure your CSV file is in the <a href="%s">correct format</a>. If it is out of order, your fields will not be imported correctly. ', 'wp-crm-system-import-tasks' ), array(  'a' => array( 'href' => array() ) ) ), esc_url( $url ) );
							echo $link; ?>
						</td>
					</tr>
				</form>
				<form id="wpcrm_export_tasks" name="wpcrm_export_tasks" method="post" action="">
					<tr>
						<td>
							...
							<input type="submit" name="export_tasks" value="<?php _e( 'Export Tasks', 'wp-crm-system-import-tasks' ); ?>" />
						</td>
						<td></td>
					</tr>
				</form>
